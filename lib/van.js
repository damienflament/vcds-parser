 ...
 * function.
 *
 * To be used in JSX components, it takes care of the special property
 * *children*. If present, its value is returned as the children array.
 *
 * Returns an array whose the first element is the properties object and the
 * second one is the children array.
 *
 * @example
 *
 ...
 */
const args = args => {
  let children = [...args] // Working on a copy because we will use shift()
  ...
  if (props.children) {
    // When there is only 1 child, JSX doesn't set an array. We take care of it
